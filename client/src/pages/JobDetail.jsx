...
setIsFetching(false);
  ...
  const permanentPhoneNumber = "9846900168";

  const handleApplyNow = () => {
    if (/^(\+|\d)[0-9]{7,}$/.test(permanentPhoneNumber)) {
      window.location.href = `tel:${permanentPhoneNumber}`;
    } else {
    ...
    <div className='container mx-auto'>
      <div className='w-full flex flex-col md:flex-row gap-10'>
        ...
        {user?._id === job?.company?._id ? (
          ...
          <CustomButton
            title='Apply Now'
            onClick={handleApplyNow} // Add this onClick handler
